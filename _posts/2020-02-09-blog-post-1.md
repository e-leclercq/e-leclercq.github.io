...
If you have not wiped the drive, you will need to delete the existing partitions (see warning above).
Once you have done this, create a partition that is `512 M` in size, label it as `EFI System`, and give it the `boot` flag.
Next you can either make the swap partition or the root partition.
The swap partition should be the two times the size of your computer's RAM and the root partition can be whatever size you would like, so long as you have room.
...
You must now mount the partitions you created to `/mnt`.
Go ahead and create the mount point for your EFI partition:
```
# mkdir /mnt/boot/EFI
```

Mount the partitions to `/mnt` (making sure that the correct drives are mounted to their respective mount points):
```
# mount /dev/sdXY /mnt
...
```
# pacstrap /mnt base base-devel linux linux-firmware nano man-db
```
...
Use `nano` to uncomment any locales you wish to use in `/etc/locale.gen` with (for the US this is `en_US.UTF-8 UTF-8`):
```
# nano /etc/locale.gen
```
...
```
Then use `nano` to edit `/etc/hosts`, making it look like the following:
```
# nano /etc/hosts
...
```
# pacman -S grub efibootmgr os-prober mtool dosfstools 
```
...
By using Arch Linux, you have entered into a world of almost infinite possibility.
I will be writing some more guides soon on some of the things that you can do with Arch and cool ways to customize it to your liking.
